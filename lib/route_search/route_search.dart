...
class _RouteSearchState extends State<RouteSearch> {
  ...
  void _search() async {
    ...
    if (_fromController.text == _toController.text) {
      setState(() {
        error = 'The stations must be different';
        _isSearching = false;
      });
      return;
    }
    ...
    final to = await db
        .collection('stops')
        .where("names", isEqualTo: _toController.text.trim().toLowerCase())
        .get();
          ...
          children: [
            ...
            TextField(
              ...
              decoration: const InputDecoration(
                  label: Text("Starting Point"), border: OutlineInputBorder()),
            ),
            const SizedBox(height: 20),
            TextField(
              controller: _toController,
              decoration: const InputDecoration(
                  label: Text("Arrival Point"), border: OutlineInputBorder()),
            ),
            const SizedBox(height: 20),
            _isSearching
                ? Shimmer.fromColors(
                ...
                : WideButton(callback: _search, text: "Find Route"),
            Text(
              error,
